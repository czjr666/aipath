...
for chunk in stream:                     <span class="cm"># 每个 chunk 带着刚生成的几个字</span>
    ...
    footnoteSourceNote: (
      <>
        ...
        、{' '}
        <a href="https://docs.anthropic.com/en/api/getting-started" target="_blank" rel="noreferrer">
          Anthropic API
        </a>
        ；具体参数与价格以官方文档为准。
      </>
    ...
    seg4P: <>第 17 课埋的伏笔在这里兑现：<b>API 服务器完全无状态（stateless）</b> —— 每个请求都是孤立的，处理完即忘，它连"上一轮"这个概念都没有。想"接着聊"，唯一的办法是你自己维护 messages 列表，<b>每一轮把全部历史原封不动重发一遍</b>。两个直接推论：一，越聊越贵（历史越长，输入 token 越多）；二，把历史从 list 里删掉，它就当场"失忆"。这两件事，下面的交互演示都能亲手验证。</>,
    seg5Title: '流式输出：体感快的秘密',
    seg5P: <>第 10 课讲过：模型本来就是<b>一个 token 一个 token</b> 往外吐的。非流式 = 等它全部吐完，打包发给你；流式输出（streaming）= 吐一点立刻推一点。<b>总耗时几乎一样，但"第一个字出现的时间"天差地别</b> —— ChatGPT 的打字机效果不是装酷，是体验刚需。眼见为实：</>,
    ...
    bridgeLead: '这一课的模型住在云端：断网就用不了，每次调用都按 token 计费，数据还得发出门。如果想让模型真正跑在自己电脑上——离线、免费、数据不出门——该怎么办？下一课就动手：用 Ollama 在本地跑开源大模型，顺便搞懂 7B、Q4 这些“黑话”到底是什么意思。',
    bridgeSteps: ['第一个 API 请求跑通', '但模型在云端、要联网花钱', '想让它跑在本地', '下一课：本地跑大模型'],
    quizTitle: '✍️ 小练习',
...
for chunk in stream:                     <span class="cm"># each chunk carries the few chars just generated</span>
    ...
    footnoteSourceNote: (
      <>
        ...
        ; exact parameters and pricing follow the official docs.
      </>
    ),
    ...
    bridgeTitle: '➡️ How This Leads to Lesson 27',
    bridgeLead: 'This lesson\'s model lives in the cloud: no internet, no use; every call is billed by token; and your data has to leave home. What if you want the model to actually run on your own computer — offline, free, data never leaving? The next lesson gets hands-on: run an open-weight model locally with Ollama, and finally decode jargon like "7B" and "Q4."',
    bridgeSteps: ['First API request works', 'But the model is cloud-bound', 'Want it running locally', 'Next: Running models locally'],
  ...
  return (
    <>
      ...
      >
        ...
        <p className="footnote source-note">{c.footnoteSourceNote}</p>
      </Lsec>
