...
function tempZone(t) {
  if (t < 0.5) return { label: '严谨', cls: '' }
  if (t <= 1.1) return { label: '平衡', cls: '' }
  return { label: '放飞', cls: 'hot' }
}
...
export default function LLMHero() {
  ...
  const cands = useMemo(() => buildCands(si, pos), [si, pos])
  ...
  useEffect(() => {
    ...
    if (phase === 'think') {
      ...
      tm = setTimeout(() => {
        clearInterval(jt)
        const q = adjust(buildCands(si, pos), tempRef.current)
        let r = Math.random()
        let idx = 0
  ...
  return (
    <div className="llm-hero" ref={rootRef}>
      ...
      <div className="lh-status">
        ...
        {done ? '生成完毕，换一句…' : `正在猜第 ${pos + 1} / ${total} 个字`}
        <span className="lh-status-r">第 15 课 · 下一个字的游戏</span>
      </div>
      ...
      <div className="lh-main">
        ...
        <div className="lh-side">
          <div className="lh-side-label">它脑中的候选 · 下一个字</div>
          {cands.map((c, i) => {
            const q = probs[i]
          ...
          <div className="lh-temp">
            ...
            />
            <div className="lh-temp-hint">往右拧，看它开始胡说八道 →</div>
          </div>
        </div>
      </div>
    </div>
